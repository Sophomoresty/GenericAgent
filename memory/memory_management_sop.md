...
**触发时机**：每次任务收尾时（用户说"好了"/任务明显完成/long_term_update被调用时），**必须自检以下清单**：

### 自检清单（任一命中 → 提议生成 L3 SOP）

| # | 条件 | 典型场景 |
|---|------|---------|
| 1 | 同一问题踩坑 ≥ 2 次才解决 | 调试反复失败、配置反复调 |
| 2 | 涉及 ≥ 3 步/≥ 2 工具协同 | 跨文件改+验证+部署 |
| 3 | 产生了可复用的排查路径 | 发现了隐蔽的前置条件 |
| 4 | 用户明确说"以后也要这样" | 显式要求保留经验 |
| 5 | 涉及非直觉的边界/约束 | 平台差异、版本陷阱、隐藏依赖 |
| 6 | 完成耗时 ≥ 15 分钟（约 8 轮以上） | 复杂度高，遗忘成本高 |
...
自检清单命中？
  ├─ NO → 正常收尾，不生成 SOP
  └─ YES → 
       1. 向用户报告: "本次任务命中 SOP 生成条件 #N，建议生成 L3 SOP"
       2. 等用户确认（不强制生成，避免垃圾 SOP）
       3. 用户同意 → 创建精简 SOP (关键前置 + 典型坑 + 最短复现路径)
       4. 注册到 L1 索引
```

### 触发率验证（每月自审）

在 `global_mem.txt` 的 `[元数据]` section 维护触发统计：
...
**验证规则**：
- 每次收尾时在 working memory 计数（命中/未命中）
- 触发率 < 0.3 → 条件太严，放宽（如降阈值为 ≥2步/1次踩坑）
- 采纳率 < 0.5 → 条件太松，收紧（如提高踩坑次数要求）
- 月底自主行动时审查一次，调整阈值